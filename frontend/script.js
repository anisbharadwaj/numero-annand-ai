...
async function pollHealth() {
  // show something immediately so user doesn't think it is frozen
  if (healthRetryCount === 0) {
    showNotice(`<i class="fas fa-spinner fa-spin"></i> CONNECTING TO ANIS-AI-SHIELD CORE...`, true);
  }

  try {
    const start = Date.now();
    const res = await fetchWithTimeout(`${API_BASE_URL}/health`, {}, 8000);
    ...
    healthRetryCount = 0;
    showNotice(
      `<i class="fa-solid fa-circle-check"></i> SERVER ONLINE • ${elapsed}ms`,
      false
    );
    ...
    if (e.name === "AbortError") {
      showNotice(
        `<i class="fa-solid fa-cloud"></i> RENDER SERVER IS WAKING UP...`,
        true
      );
    } else {
      showNotice(
